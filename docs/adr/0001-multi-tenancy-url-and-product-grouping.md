...
Consequences:
- A feature moving products is a **DB flip + 301 redirect**, not a conflict —
  cheap precisely because the spec id is global. `/{org}/{oldProduct}/…/{id}`
...
4. **Hosted multi-tenant management (commercial).** Org provisioning, billing,
   seat limits, SSO/SAML, audit — the open-core boundary.
